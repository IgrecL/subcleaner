...
This script greatly reduces the size of the condensed audios, by removing all the moments where there is music and no speech for example, as well as lines that are too short. For example, with the first episode of Trigun, this script deleted around 200 lines, leaving me with a 10 minutes audio file instead of a 16 minutes one, which is like 40% of the original length.
